...
const DeviceManagement = () => {
  ...
  const fetchDevices = async () => {
    ...
    try {
      ...
      // If we get an unauthorized error, redirect to login WITHOUT removing token
      if (error instanceof Error && error.message.includes('401')) {
        console.log('Authentication error in device status update, redirecting to login without removing token');
        // Use the login page with action parameter to handle logout properly
        window.location.href = '/login?action=logout';
      }edirecting to login without removing token');
        // Use the login page with action parameter to handle logout properly
        window.location.href = '/login?action=logout';
      } else {
