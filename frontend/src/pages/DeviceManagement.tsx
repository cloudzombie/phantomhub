    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error 
          ? error.message 
          : (error as any)?.response?.data?.message || 'Failed to register O.MG Cable. Please try again.'
      );
  ...
  const registerUsbDevice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error 
          ? error.message 
          : (error as any)?.response?.data?.message || 'Failed to register USB O.MG Cable. Please try again.'
      );
  ...
  const updateDeviceStatus = async (deviceId: number, status: string) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setDevices(prev => 
          prev.map(device => 
            device.id === deviceId ? { ...device, status: status as 'online' | 'offline' | 'busy' } : device
          )
        );
      } else {
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <DeviceInfoPanel 
                  deviceInfo={{
                    name: selectedDevice.name, // Add required name property
